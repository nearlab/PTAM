...
%create camera vectors
C_vect = quatrotate(Cam_Est(:,1:4),[zeros(M,2) ones(M,1)]);
...
%creat sba out camera vectors
C_vect_sba = quatrotate(cams_sba(:,1:4),[zeros(M,2) ones(M,1)]);

%plot sba output
scatter3(struc_sba(:,1),struc_sba(:,2),struc_sba(:,3))
plot3(cams_sba(:,5),cams_sba(:,6),cams_sba(:,7))

%plot vectors of camera direction
quiver3(cams_sba(:,5),cams_sba(:,6),cams_sba(:,7),C_vect_sba(:,1),C_vect_sba(:,2),C_vect_sba(:,3),'k')

axis equal

legend('true structure','true camera path','true camera att.','sba struct.'...
    ,'sba cam path','sba att.')
