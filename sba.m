...
%rotate sba quaternions to inertial frame
refined_cams(:,1:4) = quatconj(refined_cams(:,1:4));

%convert camera coordinates back to inertial
R = quat2dcm(refined_cams(:,1:4));

C_tilde2 = cams(:,5:7);

for ii = 1:M
    refined_cams(ii,5:7) = (-R(:,:,ii)'*refined_cams(ii,5:7)')';
end
